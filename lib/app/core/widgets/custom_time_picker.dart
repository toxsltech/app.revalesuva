        ...
        return Column(
          ...
          children: <Widget>[
            ...
            Row(
              ...
              children: <Widget>[
                if (hourDialType == _HourDialType.twelveHour && timeOfDayFormat == TimeOfDayFormat.a_space_h_colon_mm) const _DayPeriodControl(),
                Expanded(
                  child: Row(
                ...
                /*if (hourDialType == _HourDialType.twelveHour && timeOfDayFormat != TimeOfDayFormat.a_space_h_colon_mm) ...<Widget>[
...
class _DayPeriodControl extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final OutlinedBorder resolvedShape = (timePickerTheme.dayPeriodShape ?? defaultTheme.dayPeriodShape).copyWith(side: resolvedSide);
...
class _AmPmButton extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final Color resolvedBackgroundColor =
        MaterialStateProperty.resolveAs<Color>(timePickerTheme.dayPeriodColor ?? defaultTheme.dayPeriodColor, states);
    final Color resolvedTextColor =
        MaterialStateProperty.resolveAs<Color>(timePickerTheme.dayPeriodTextColor ?? defaultTheme.dayPeriodTextColor, states);
    final TextStyle? resolvedTextStyle =
        MaterialStateProperty.resolveAs<TextStyle?>(timePickerTheme.dayPeriodTextStyle ?? defaultTheme.dayPeriodTextStyle, states)
...
class _DialPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final double handleRadius =
        clampDouble(labelRadius - (radius < 0.5 ? 1 : 0) * (labelRadius - innerLabelRadius), _kTimePickerDialMinRadius, double.infinity);
...
class _DialState extends State<_Dial> with SingleTickerProviderStateMixin {
      ...
      if (themeData.useMaterial3)
        for (final TimeOfDay timeOfDay in _twentyFourHours)
          _buildTappableLabel(
            ...
            label: timeOfDay.hour != 0 ? '${timeOfDay.hour}' : localizations.formatHour(timeOfDay, alwaysUse24HourFormat: true),
            onTap: () {
  ...
  Widget build(BuildContext context) {
    ...
    final Color dialTextUnselectedColor =
        MaterialStateProperty.resolveAs<Color>(timePickerTheme.dialTextColor ?? defaultTheme.dialTextColor, <MaterialState>{});
...
class _TimePickerInputState extends State<_TimePickerInput> with RestorationMixin {
  ...
  int? _parseHour(String? value) {
    ...
    if (MediaQuery.alwaysUse24HourFormatOf(context)) {
      ...
      if (newHour > 0 && newHour < 13) {
        if ((_selectedTime.value.period == DayPeriod.pm && newHour != 12) || (_selectedTime.value.period == DayPeriod.am && newHour == 12)) {
  ...
  Widget build(BuildContext context) {
    assert(debugCheckHasMediaQuery(context));
    final TimeOfDayFormat timeOfDayFormat =
        MaterialLocalizations.of(context).timeOfDayFormat(alwaysUse24HourFormat: _TimePickerModel.use24HourFormatOf(context));
...
class _HourMinuteTextFieldState extends State<_HourMinuteTextField> with RestorationMixin {
  ...
  Widget build(BuildContext context) {
    ...
    final Color startingFillColor =
        timePickerTheme.inputDecorationTheme?.fillColor ?? timePickerTheme.hourMinuteColor ?? defaultTheme.hourMinuteColor;
    ...
    final TextStyle effectiveStyle = MaterialStateProperty.resolveAs<TextStyle>(widget.style, states).copyWith(color: effectiveTextColor);
...
class _TimePickerDialogCustomState extends State<TimePickerDialogCustom> with RestorationMixin {
  late final RestorableEnum<TimePickerEntryMode> _entryMode =
      RestorableEnum<TimePickerEntryMode>(widget.initialEntryMode, values: TimePickerEntryMode.values);
  ...
  final RestorableEnum<AutovalidateMode> _autovalidateMode =
      RestorableEnum<AutovalidateMode>(AutovalidateMode.disabled, values: AutovalidateMode.values);
    ...
    switch (_entryMode.value) {
      ...
      case TimePickerEntryMode.dialOnly:
        switch (orientation) {
          ...
          case Orientation.landscape:
            timePickerSize = Size(_kTimePickerLandscapeSize.width * textScaleFactor,
                  ...
                  children: <Widget>[
                    ...
                    /*TextButton(
                      ...
                      child: Text(widget.cancelText ??
                          (theme.useMaterial3 ? localizations.cancelButtonLabel : localizations.cancelButtonLabel.toUpperCase())),
                    ),*/
                    TextButton(
...
class _TimePickerState extends State<_TimePicker> with RestorationMixin {
  Timer? _vibrateTimer;
  late MaterialLocalizations localizations;
  final RestorableEnum<_HourMinuteMode> _hourMinuteMode = RestorableEnum<_HourMinuteMode>(_HourMinuteMode.hour, values: _HourMinuteMode.values);
  final RestorableEnumN<_HourMinuteMode> _lastModeAnnounced = RestorableEnumN<_HourMinuteMode>(null, values: _HourMinuteMode.values);
  ...
  Widget build(BuildContext context) {
    ...
    switch (widget.entryMode) {
      ...
      case TimePickerEntryMode.dialOnly:
        helpText =
            widget.helpText ?? (theme.useMaterial3 ? localizations.timePickerDialHelpText : localizations.timePickerDialHelpText.toUpperCase());
      ...
      case TimePickerEntryMode.inputOnly:
        final String helpText =
            widget.helpText ?? (theme.useMaterial3 ? localizations.timePickerInputHelpText : localizations.timePickerInputHelpText.toUpperCase());
...
class _TimePickerDefaultsM2 extends _TimePickerDefaults {
  ...
  Color get _hourMinuteInputColor {
    return MaterialStateColor.resolveWith((Set<MaterialState> states) {
      return states.contains(MaterialState.selected) ? Colors.transparent : _colors.onSurface.withOpacity(0.12);
    });
  }
...
class _TimePickerDefaultsM3 extends _TimePickerDefaults {
  ...
  InputDecorationTheme get inputDecorationTheme {
    ...
    final BorderRadius selectorRadius =
        const RoundedRectangleBorder(borderRadius: BorderRadius.all(Radius.circular(8.0))).borderRadius.resolve(Directionality.of(context));
    return InputDecorationTheme(
      contentPadding: EdgeInsets.zero,
      filled: true,
